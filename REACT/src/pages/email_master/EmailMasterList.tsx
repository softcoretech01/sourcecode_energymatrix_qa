...
export default function EmailMasterList() {
  ...
  const fetchEmails = async () => {
    try {
      ...
      setEmailData(Array.isArray(data)
        ? data.map((item: any) => ({ ...item, status: Number(item.status) === 1 ? 1 : 0 }))
        : []
      );
    } catch (error) {
  ...
  const toggleEmailStatus = async (row: any) => {
    const newStatus = Number(row.status) === 1 ? 0 : 1;
    try {
      const payload = {
        email_id: row.email_id,
        email_category: row.email_category,
      ...
      await api.put(`/email-master/${row.id}`, payload);
      setEmailData((prev) =>
        prev.map((item: any) => (item.id === row.id ? { ...item, status: newStatus } : item))
      );
    } catch (error) {
      console.error("Email status toggle failed", error);
                      ...
                      <TableRow key={row.id} className="hover:bg-slate-50 bg-white">
                        ...
                        <TableCell className="py-2 text-center">
                          <div className="flex justify-center gap-1 items-center">
                            <StatusSlider
                              status={row.status}
                              onToggle={() => toggleEmailStatus(row)}
                            />
                            <Button
                              variant="ghost"
                              size="icon"
