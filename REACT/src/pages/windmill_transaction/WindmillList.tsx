...
import { Search, Edit, Calendar as CalendarIcon } from "lucide-react";
            ...
            {wm.windmill_number}
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        ...
                                        {fromDate ? format(fromDate, "P") : <span className="text-xs">From Date</span>}
                                    </Button>
                                </PopoverTrigger>
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        <CalendarIcon className="mr-2 h-3.5 w-3.5" />
                                        {toDate ? format(toDate, "P") : <span className="text-xs">To Date</span>}
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-auto p-0">
                                    <Calendar
                                    ...
                                    {filteredData.length > 0 ? (
                                        filteredData.map((row) => (
                                            <TableRow key={row.id} className="hover:bg-slate-50">
                                                <TableCell className="py-2 text-sm">{row.transaction_date}</TableCell>
                                                ...
                                                <TableCell className="py-2 text-center">
                                                    <div className="flex justify-center gap-2">
                                                        <Button variant="ghost" size="icon" disabled={row.status === "Posted"} className={cn("h-6 w-6", row.status === "Posted" ? "text-gray-400 cursor-not-allowed" : "text-primary hover:text-primary hover:bg-primary/10")} onClick={() => navigate(`/windmill/edit/${row.id}`)}>
