...
import { BACKEND_UPLOAD_URL } from "@/services/api";
...
export default function EBStatementSolarPdf() {
    ...
    useEffect(() => {
        const fileFromQuery = params.get("file");
        const fileFromSession = sessionStorage.getItem("ebStatementSolarFile");
        const filename = fileFromQuery || fileFromSession;

        if (filename) {
            setPdfUrl(`${BACKEND_UPLOAD_URL}/eb_statements/${filename}`);
        }

        const parsed = sessionStorage.getItem("ebStatementSolarData");
        if (parsed) {
            try {
                setParsedData(JSON.parse(parsed));
            } catch (err) {
