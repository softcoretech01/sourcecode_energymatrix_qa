                                        ...
                                        filteredData.map((row, index) => (
                                            <TableRow key={row.id || index} className="hover:bg-slate-50 bg-white">
                                                ...
                                                <TableCell className="py-2">
                                                    {/** use is_submitted flag for S/P indicator, fall back to status text */}
                                                    <Badge
                                                        className={cn(
                                                            "font-bold w-6 h-6 rounded flex items-center justify-center p-0 border-none",
                                                            (row.is_submitted === 1 || row.status === "Posted" || row.status === 1)
                                                                ? "bg-emerald-600 hover:bg-emerald-700 text-white"
                                                                : "bg-red-600 hover:bg-red-700 text-white"
                                                        )}
                                                    >
                                                        {(row.is_submitted === 1 || row.status === "Posted" || row.status === 1) ? "P" : "S"}
                                                    </Badge>
                                                </TableCell>
