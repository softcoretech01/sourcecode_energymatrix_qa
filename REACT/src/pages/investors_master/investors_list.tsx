...
export default function InvestorsList() {
    ...
    const toggleInvestorStatus = async (row: any) => {
        const newStatus = Number(row.status) === 1 ? 0 : 1;
        const token = localStorage.getItem("access_token");
        try {
            const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/investors/update/${row.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
                ...
                body: JSON.stringify({
                    ...
                    share_quantity: row.shareQuantity,
                    status: newStatus,
                }),
            });
            if (!res.ok) {
            ...
            console.error("Investor status update failed", err);
                                            ...
                                            <TableRow key={row.id} className="hover:bg-slate-50 bg-white">
                                                ...
                                                <TableCell className="py-2 text-center">
                                                    <div className="flex justify-center gap-1 items-center">
                                                        <StatusSlider
                                                            status={row.status}
                                                            onToggle={() => toggleInvestorStatus(row)}
                                                        />
                                                        <Button
                                                            variant="ghost"
                                                            size="icon"
